...
class heuristic2D(object):
    ...
    def __init__(self, step=10, diagonal=14, scale=1):
        """
        Attributes:
    ...
    def octile(self, u, v):
        ...
        dx = abs(u[0] - v[0])
        dy = abs(u[1] - v[1])
        distance = self.STEP*max(dx, dy) + (self.DIAG - self.STEP)*min(dx, dy)
        return distance
